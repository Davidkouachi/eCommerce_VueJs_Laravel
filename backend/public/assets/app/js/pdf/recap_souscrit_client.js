$(document).ready(function () { 
    ...
    async function loadAllData() {
        try {
            ...
            if (!date1.trim() || !date2.trim() || !client.trim()) {
                showAlert("Alert", "Veuillez vérifier les champs.", "info");
                return false;
            }

            preloaderBar();

            const countResponse = await fetchCount(date1, date2, client, host, db, user, mdp);
            ...
            for (let page = 1; page <= totalPages; page++) {
                const pageData = await fetchPage(page, date1, date2, client, host, db, user, mdp, perPage);
    ...
    function fetchCount(date1, date2, client, host, db, user, mdp) {
        return $.ajax({
            url: $('#url').attr('content') + '/api/count_liste_recap_souscrit_client_detail',
            method: 'GET',
            data: { date1, date2, client, host, db, user, mdp }
        });
    }

    function fetchPage(page, date1, date2, client, host, db, user, mdp, perPage) {
        return $.ajax({
            url: $('#url').attr('content') + '/api/liste_recap_souscrit_client_detail_page',
            method: 'GET',
            data: { date1, date2, client, host, db, user, mdp, page, perPage }
        });
    }
    ...
    function PDF_Facturation() 
    {
        ...
        const pdfFilename = "Etat de gestions des récapitulatifs des souscriptions des clients du " + formatDate(date1_pdf) + " au " + formatDate(date2_pdf);
        doc.setProperties({
            title: pdfFilename,
        });
        ...
        function drawConsultationSection(yPos) {
            ...
            const title = "RECAPITULATIF DES SOUSCRIPTIONS DES CLIENTS DU " + formatDate(date1_pdf) + " AU " + formatDate(date2_pdf);
        ...
        function addFooter() {
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.setFontSize(5);
                doc.setFont("helvetica", "normal");
                const title_footer = "ETAT DU RECAPITULATIF DES SOUSCRIPTIONS DES CLIENTS DU" + formatDate(date1_pdf) + " AU " + formatDate(date2_pdf);
